...
## 2026-09-10 — Phase 3 Primary Project implementation

The owner explicitly started Phase 3. WIP remains 1; Phase 4 is not started. No unresolved domain-model decision was identified: the locked prompt already requires a separate project/container entity, one persisted Primary routing choice, exactly one non-null container per capture, and semantic reassignment.

Use an additive singleton `capture_routing` table for Primary. Creating a project does not implicitly make it Primary. Set/clear Primary affects subsequent commits only; it never moves old captures. Resolve Primary inside the existing create-capture transaction, with Waiting Room fallback. Project IDs use UUIDv7; display names are trimmed/nonblank, and identity remains the ID rather than an invented unique-name constraint. No new capture-record writer is introduced; Gate 11 validation remains in the same constructor.

Expose minimal project creation and Make/Clear Primary in the existing context panel, and a deliberate Move form in the engaged editor. Keep the capture destination as ambient feedback. Gate 6 was explicitly re-inspected because QuickCapture.tsx changed: context loads independently of readiness and no project/metadata prompt is added. Automated simulated-IPC checks supplement, but do not replace, the accepted code-inspection gate.

Reassignment patches only container_id and semantic time. Text autosave patches content/title under the same database lock and cannot restore a stale container. The editor flushes its current buffer before moving. Invalid destinations roll back; same-container moves are no-ops. Presentation state stays separate.

Native verification exposed a retrieval issue that blocked reaching the editor: hover-driven height changes re-centered the native panel and moved the target row before a click. Reserve preview space for nonempty lists and serialize native geometry requests; do not resize/refocus merely when the hovered preview changes. Panel heights include space for the minimal project controls at the tested Windows text scaling and remain capped at 420 logical pixels. This is a Phase 3 usability correction, not Phase 4 hierarchy or a restyle of the ambient material. Active-card dimensions/compositing/inward treatment are unchanged.

## 2026-09-10 — Phase 3 verification result

The four-target Release workflow passed on application commit 93e1f5a; the workflow itself required no revision. Native create/set/routed commit/move/restart/unset/fallback checks passed in an isolated Windows test database. Current readiness qualification is 39.9420–84.8650ms across all 14 samples (0 above 100ms), including three process lifetimes. Preserve the earlier 128.9790ms failure as historical evidence: this retest does not establish its cause or permanent elimination, and Phase 3 did not change native hotkey/focus orchestration. The open editor appearance/default-size check is now observed; physical rounding at 125% is reported explicitly in STATUS. Phase 4 remains unstarted.

## 2026-09-10 — Phase 4 bounded navigation and cumulative latency watch

The owner accepted Phase 3 and authorized Phase 4. WIP is 1; Phase 5 and anchor/reveal visual polish remain deferred. The layout choice was flagged before implementation: use two fixed vertical content areas inside the existing 350x420 logical expanded window. Hover changes only the other area. Rows and their scroll position in the source area remain fixed, including when hovering siblings, changing child counts or showing long previews. Earlier layers compress into fixed header controls for the current ancestor path and direct sibling navigation. The path control exposes ancestors along the current path, not the entire hierarchy. Deliberate header clicks or form submissions may recompose both areas.

Populate each list from a snapshot, retaining it while that area is the hover source. Incoming context events do not reorder populated rows under the pointer. Reopening the panel or deliberately navigating refreshes lists; existing open lists can therefore show stale counts/content until navigation. Per-area request revisions reject late responses that would otherwise overwrite a more recent sibling selection. Expanded native geometry changes only when reveal/held-focus state changes, never because of hover, depth or preview length. This generalizes the Phase 3 correction at the earlier decision rather than treating it as a one-off.

Use the existing containers.parent_id field for nested project contexts; no new entity, schema migration, reparenting feature or capture writer is introduced. New children require an existing project parent. Waiting Room and missing parents are rejected. Generated IDs make the create-only parent relationship acyclic; this is not a claim that arbitrary external SQL edits are validated. Nested Primary still routes to exactly one container, without inherited ancestor membership. Capture provenance continues through the sole validated constructor. Navigation and presentation must not change semantic timestamps; explicit reassignment retains its existing timestamp behavior.

Capture readiness is cumulative. CAPTURE_READINESS.md retains every recovered raw event, including earlier development failures and the abandoned gutter experiments, with source log and line. Current ledger: 49 samples, maximum 258.1492ms, six above 100ms. Phase 4 adds 61.3282ms and 44.9857ms; no new exceedance was observed. The 128.9790ms inward-restart2 failure is unexplained and remains on standing watch. A new phase's passing samples do not resolve it. CPU load/memory pressure were not measured, so no causal explanation is inferred. The hotkey/focus implementation, inward card styling, window compositing, release workflow and installer identity were not changed in Phase 4.

Phase 4 release verification passed on application commit 6c234b1, workflow run 34571415468, all four installer targets and 21 Rust tests per runner. Actual built Windows MSI identity/upgradeCode was inspected and preserved. No workflow revision was required. This establishes build continuity, not installed-upgrade or cross-platform native UI verification. The owner receives the implementation for review with the standing latency watch and native interaction caveats preserved in STATUS; Phase 5 remains unstarted.
